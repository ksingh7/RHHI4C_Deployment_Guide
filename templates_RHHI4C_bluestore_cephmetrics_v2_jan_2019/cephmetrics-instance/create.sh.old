...
openstack network create $PUB_NET_NAME --provider-physical-network $EXT_PROVIDER_PHYSICAL_NET \
                                --provider-network-type $EXT_PROVIDER_NET_TYPE \
                                --external --share

openstack subnet create --network $PUB_NET_NAME $PUB_SUBNET_NAME --subnet-range $EXT_PROVIDER_NET_CIDR \
                         --allocation-pool $EXT_PROVIDER_NET_ALLOCATION_POOL \
                         --dns-nameserver $DNS_IP --gateway $EXT_PROVIDER_NET_GATEWAY \
...
openstack network create $PROVIDER_NET_NAME --provider-physical-network $EXT_PROVIDER_PHYSICAL_NET  \
